...
export default class FullStoryCard extends BindingClass {
  ...
  async fullStory() {
    ...
    try {
      ...
      const storyCard = this.createFullStoryCard(
        storyData,
        author,
        currentUser
      );
  ...
  createFullStoryCard(story, author, currentUser) {
    ...
    const inFavorites =
      currentUser && currentUser.favorites.includes(story.storyId);
    const inDislikedStories =
      currentUser && currentUser.dislikedStories.includes(story.storyId);
      ...
      if (currentUser.userId === author.userId) {
        ...
        if (inFavorites) {
          ...
          unlikeButton.addEventListener("click", (event) => {
            ...
            this.submit.unsubmitLike(story.storyId);
          });
          ...
          undislikeButton.addEventListener("click", (event) => {
            ...
            this.submit.unsubmitDislike(story.storyId);
          });
  ...
  async handleDelete(storyId) {
    try {
      ...
      if (success) {
        ...
        currentUser.storiesWritten = currentUser.storiesWritten.filter(
          (id) => id !== storyId
        );
